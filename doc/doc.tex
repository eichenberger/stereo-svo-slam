...
For testing the algorithm with a reference implementation we use two synthetic scenes generated by Blender. The advantage of having synthetic scenes is that we can test some well defined movements where we know exactly the trajectory. We compare our implementation with svo edgelet \cite{svo_edgled}. This is based on the reference svo implementation. However, it is using newer versions of OpenCV, libegien, etc. They added edgelets tracking which is missing in the original svo sources and they added a GUI to observe the tracking. However, it's much slower than the original implementation on the other side it seems to be more robust than the original implementation.

\sectuion{Classroom}
In our first test we change the position and angles as shown in figure \ref{fig:blender_classroom_traj}.

\begin{figure}[H]
  \includegraphics[width=1.0\textwidth]{img/blender_classroom_traj.png}
  \caption{Blender classroom trajectory}\label{fig:blender_classroom_traj}
\end{figure}

We now measure the difference of the reference trajectory to what we measure by using our SVO implementation. We see in figure \ref{fig:blender_classroom_our_diff} that we have low errors (< 0.06 m) when moving and up to 2° for angles.
\begin{figure}[H]
  \includegraphics[width=1.0\textwidth]{img/blender_classroom_our_diff.png}
  \caption{Blender classroom error our implementation. Distances in meter, angels in °.}\label{fig:blender_classroom_our_diff}
\end{figure}

Figure \ref{fig:blender_classroom_ref_diff} shows the difference of the reference implementation. Please not that this implementation only uses monocular images. The first 11 pictures are required to initialize the tracking. Therefore, no data is available during that time. The scaling is not known and therefore the output trajectory was later scaled to match in meters. It seems that the point cloud is less accurate than in the stereo case. A movement along x axis also affects the yaw value. The simultainous change in movement and angle seems to be a huge problem for the monocular tracker. The tracker can't insert new keypoints during this movement because the change in position is too small. Therefore, we see a huge error there.
\begin{figure}[H]
  \includegraphics[width=1.0\textwidth]{img/blender_classroom_ref_diff.png}
  \caption{Blender classroom error reference implementation. Distances in meter, angels in °.}\label{fig:blender_classroom_ref_diff}
\end{figure}
