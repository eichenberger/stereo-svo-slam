...
Lets assume we have an image I1 at time t-1 and an image I2 at time t. We know the position of a point P1 in image 1. We use Lucas Kanade Optical Flow to find the position P2 of the point in image 2. It does that by minimizing the intensity difference between I1 at P1 and I2 at P2. Figure \ref{fig:optical_flow} shows the problem we want to solve. We call the change in position of the point optical flow.

\begin{figure}[H]
	\includegraphics[width=1.0\textwidth]{img/optical_flow.png}
	\caption{Optical Flow: Where did point at position P1 in image I1 move in image I2?}\label{fig:optical_flow}
\end{figure}

\section{Lucas Kanade Intuitive}

Lucas Kanade Optical Flow tries to answer where a point moves between two images as shown in figure \ref{fig:optical_flow}. In the simplest case it just tells us where a certain pixel was in the image before and where it is now. It does that in an efficient way by using intensity gradients. This section describes how this works in an intuitive but simplified way while the next section will describe it in a more mathematical way.\\
...
What we can see in \ref{fig:optical_flow_intuitive} (b) that it's impossible to track movements over bigger distances. If the patch in the reference image does not connect to where the patch moved, we are unable to find the right gradient. As we see it's even possible that we move in the wrong direction. To reduce this problem we can downsample the image as shown in (c). For illustration reasons we reduced the resolution only in horizontal direction. In reality the whole image would be downsampled by factor 2,4,8,16, etc. The array of images with different resolutions is called an optical pyramid. As we see in (c) after we downsampled the image by factor two we have again a valid gradient which points in the right direction. The idea is that we start to optimized on the lowest level. If we converge we optimize one level up and so on. With this idea it is possible to track flows over higher distances while still being efficient.

\begin{figure}[H]
  \centering
  \subfloat[Small move where we find the right gradient green]{\includegraphics[height=0.2\textheight]{img/optical_flow_intuitive_1.png}}
	\quad
  \subfloat[Bigger move where we find an invalid gradient red]{\includegraphics[height=0.2\textheight]{img/optical_flow_intuitive_2.png}}
	\quad
	\subfloat[Bigger move as in b but down sampled in horizontal direction. We find the right gradient green]{\includegraphics[height=0.2\textheight]{img/optical_flow_intuitive_3.png}}
...
We extend the above scenario from a one dimensional movement to a two dimensional movement as shown in figure \ref{fig:optical_flow_2d}. We calculate the gradient in the vertical direction as well. By doing this we can track movements in x and y direction. We see that not all values in the patch help to find the right gradient. Some gradients in the patch point in the wrong direction. However, in total the gradient points in the right direction. By using image pyramids we can again reduce the influence of such miss matches.
\begin{figure}[H]
...
In this chapter we discuss the implementation that was written during this thesis. It should give a ruff overview of the code base. All the source code written during this thesis as well as the document sources can be found on github under the following link:\\
...
The source code uses GNU Makefiles for compilation. Table \ref{tab:envvars} shows the environment variables be used to control the make process.
\begin{table}[H]
  \centering
  \begin{tabular}{|l|l|}
...
All applications beside the qt-viewer accept some command line arguments. It is possible to query the help text by providing a -h argument:\\
\begin{lstlisting}
./slam_app -h
...
It is possible to access via Websocket URLs. Table \ref{tab:websocketapi} shows the implemented commands and URLs. For the trajectory it will send x1, y1, z1, rx1, ry1, rz1, x2, y2, z2, \dots The test application will open a Websocket on port 8001.
