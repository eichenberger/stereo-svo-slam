...
Because equation \ref{eq:pe_problem} is the same as for the inverse compositional problem it is clear that also the taylor approximation stays the same shown equation \ref{eq:iclk_taylor}. However, it is clear that this time our warp $W$ must be different. We therfore write 

We want to find $\xi$ which now represents our pose change so that the intensity difference gets minimal. We therefore deviate along $\xi$ which leads us to \ref{eq:se3_jacobian}.
\begin{equation}\label{eq:se3_jacobian}
	\frac{e^{\xi}*P*p}{\delta \xi}=
	\begin{pmatrix}
		f_x & f_y
	\end{pmatrix}
	\begin{pmatrix}
