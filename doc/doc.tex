...
\begin{figure}
  \centering
	\subfloat[Sparse Image Alignment with previous frame $I_{k-1}$ and current frame $I_{k}$. Keypoint position u' by projecting p with $T_{k,k-1}$ to the image]{\includegraphics[scale=0.4]{img/pose_estimation_sparse.png}}
	\quad
	\subfloat[Optical Flow with keyframes $I_{r1,2}$. New keypoint position u' found with optical flow]{\includegraphics[scale=0.4]{img/pose_estimation_opt_flow.png}}
	\quad
	\subfloat[Pose $T_{\omega,k}$ refinement based on difference between keypoint position from sparse image alignemnt and optical flow]{\includegraphics[scale=0.4]{img/pose_estimation_refinement.png}}
...
If we know the pose of the camera we can transform the depth to the global coordinate system. First we need to calculate the X and Y position in local camera coordinates. This is shown in equation \ref{eq:depth_xy}.
...
Figure \ref{fig:optical_flow_intuitive}b shows that we can't track points that moved a lot. If the patch in the reference image does not connect to where the patch moved, we are unable to find the correct gradient. To solve this problem we can downsample the images as shown in figure \ref{fig:optical_flow_intuitive}c. We again find a valid gradient. For optical flow we use image pyramids \cite{rvc} which store downsampled images by factor 1,2,4,8,\dots. We then start to optimized at the lowest level and if we converge we move one level up, etc. With this idea it is possible to track movements over big distances.

\begin{figure}[H]
  \centering
  \subfloat[Small move where we find the correct gradient green]{\includegraphics[height=0.2\textheight]{img/optical_flow_intuitive_1.png}}
	\quad
  \subfloat[Bigger move where we find an invalid gradient red]{\includegraphics[height=0.2\textheight]{img/optical_flow_intuitive_2.png}}
	\quad
	\subfloat[Bigger move as in b but down sampled in horizontal direction. We find the correct gradient green]{\includegraphics[height=0.2\textheight]{img/optical_flow_intuitive_3.png}}
...
All applications beside the qt-viewer accepts some command line arguments. It is possible to query the help text by calling the application with -h:\\
\begin{lstlisting}
./slam_app -h
...
The pose refiner updates the pose by using the keyframes where a keypoint first has been seen. This reduces the drift over time. It uses the algorithm described in chapter \ref{ch:pose_refinement}. Figure \ref{fig:flow_pose_refiner} shows the flow diagram of the pose refiner.
...
Additionally to the above output the test application offers a Websocket \cite{websocket} backend. Through this backend it is possible for the Qt Viewer to read 3D data like current pose, keyframes and trajectory. The Websocket idea is shown in figure \ref{fig:websocket}.
...
\begin{figure}[H]
	\centering
	\subfloat[Top View (x,z axes)]{\includegraphics[width=0.9\textwidth]{img/blender_classroom_simple_comp_top.png}}\\
	\subfloat[Side View (z,y axes)]{\includegraphics[width=0.9\textwidth]{img/blender_classroom_simple_comp_side.png}}
...
\begin{figure}[H]
	\centering
	\subfloat[Top View (x,z axes)]{\includegraphics[width=0.9\textwidth]{img/blender_barcelona_comp_top.png}}\\
	\subfloat[Side View (z,y axes)]{\includegraphics[width=0.9\textwidth]{img/blender_barcelona_comp_side.png}}
