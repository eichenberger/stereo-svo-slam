...
$\xi$ in \ref{eq:pe_lie} is the change in angle and position in Lie algebra. We can transfer from Lie algebra back to SE(3) which is our ``normal'' space by using the exponential map $e$. The exponential map is the pendant to the Euler function for matrices. In our case it hast  the closed form solution shown in \ref{eq:pe_closed_form} \cite{rvc}.
...
So instead of searching the gradient for our pose we now try to find $\xi$. We then transform it with the exponential map to get the gradient we need to update the pose for the next iteration. We can derive $e^{\xi}*p$ as shown in equation \ref{eq:se3_jacobian}.
