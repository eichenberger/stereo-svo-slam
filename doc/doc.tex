...
The mathematical detail on how we calculate the depth and the world coordinates from a stereo image we can see in chapter \ref{ch:depth}.

\section{Sparse Image Alignment}\label{sec:sia}

\begin{figure}
  \centering
	\subfloat[Sparse Image Alignment with previous frame $I_{k-1}$ and current frame $I_{k}$. Keypoint position u' found with projection with $T_{k,k-1}$]{\includegraphics[scale=0.4]{img/pose_estimation_sparse.png}}
	\quad
...
\url{https://github.com/eichenberger/stereo_svo_slam}
...
We showed that the SVO works and is faster than e.g. ORB SLAM. Our implementation does not yet achieve the same frame rate as the reference implementation. There was no time for deeper optimizations left. Some initial tests with OpenMP and TBB failed because the single threaded version was faster. It seems that the task size per iteration is too small so that the overhead of multiprocessing is too big. By increasing the task size this could probably be improved. However, more important seems to be to actively use vectorization. By consistently using OpenCV Vectors and Matrixes, the compiler can optimize the code. Already at least factor 2 was gained by doing so. The depth filter implementation is the worst part in the current implementation. However, it was also optimized the least. Therefore, we could gain the most by optimizing this part at the moment.\\
First tests on an ARM iMX8QM (Cortex A72) based system showed frame rates of around 10fps vs around 5fps for ORB SLAM. However, here no optimization or analysis was done yet. ARM NEON vectorization is used by the compiler but no tracing of the library was done yet.\\
The accuracy of our implementation is behind modern SLAM implementations. We could improve this by projection old keypoints into the current frame to have some kind of local re-localization. We don't do that at the moment because of speed concerns and to keep the algorithm simple.\\\\
We implemented a 3D viewer which can be used over a network connection. This allows us to debug devices that are further away e.g. via Wifi or Ethernet. The viewer can be optimized by asking for differences instead of querying all keyframes each time. This would reduce the bandwidth when using Wifi. However, as a proof of concept this seems to be a good approach.\\\\
We showed an easy way to generate test scenes by using Blender animations. This allows us to make debugging much easier because we know all movements in detail and we can create movements in only one direction. Such scenes help debugging algorithms a lot and make also comparing results easy. Generating such scenes is simple, cheap and quick.\\\\
...
The CPVR lab today uses ORB SLAM for their augmented reality projects. The benefit of SVO would only be performance wise but not in terms of accuracy. Therefore, the effort to port their current projects to SVO doesn't seem to be justifiable. However, for future project a combination of SVO and ORB SLAM could be an efficient solution for mobile devices. SVO could be used for tracking between keyframes, while ORB SLAM is used to do re-location. Inserting new keyframes and computing descriptors would still be done by ORB SLAM but the pose estimation would only involve sparse image alignment and pose refinement without triangulation. This could speed up the frame processing by factor 2-3.\\\\§
For embedded devices which are below modern mobile phones, SVO should be preferred over ORB SLAM. First results of our implementation proofed that it is faster than ORB SLAM. Tweaking and optimizing the code would probably allow frame rates of around 20-30 fps which is fast enough for most task.\\\\
We conclude that SVO shows great potential for the use on embedded devices. The current implementation is not ready to use yet and would need more tuning. However, it shows that it is faster than other SLAM algorithms like e.g. ORB SLAM.
