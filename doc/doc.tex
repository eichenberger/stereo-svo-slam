...
\maketitle
\begin{abstract}
  Simultaneous location and mapping (SLAM) is a technology used for robot navigation and augmented reality. Today most SLAM libraries are either proprietary or not ready for embedded systems. In this thesis we will write and analyse a library which is open source and can be used on embedded devices.
\end{abstract}

\section*{Executive Summary}
Simultaneous Location and Mapping (SLAM) is widely used for robot navigation and augmented reality. Often this algorithms need a high performance graphics card or a high performance processor. For industrial and robotics purposes we have constraints in space, temperature and power consumption. Therefore less power consuming devices with less CPU performance are required. In this thesis we implement and analyse an algorithm called SVO which the authors claim is capable to run on embedded devices. The reference implementation uses monocular cameras, while we use a stereo camera. We show that the algorithm is heavily based on the idea of optical flow which is a well known principle in computer vision. The implemented algorithm performs 4 tasks. It maintains a 3D point cloud which is used to guess the camera pose. Then it does a pose estimation based on the 3D cloud which is used as first estimated for the pose. This is called sparse image alignment and uses a modified version of optical flow which directly outputs a pose instead of a warp matrix. To make the guess more accurate it then refines the pose by using standard optical flow followed by a minimization of the re-projection error. In a last step it performs a 3D point cloud update where it tries to refine the cloud by taking the new pose into account.\\
We show that the algorithm works as described and is able to estimate the 3D pose. We will see that the algorithm performs indeed faster than a comparable SLAM algorithm called ORB SLAM. However, the current implementation is less accurate than ORB. We conclude that SVO shows great promise as a SLAM algorithm for embedded devices. However, the current implementation is not production ready and would need improvements.
